...
impl Into<String> for User {
    fn into(self) -> String {
        let claims = RegisteredClaims {
            ...
            subject: Some(serde_json::to_string(&self).expect("JSON SERIALIZATION FAILED!")),
            ..Default::default()
        };
        let key = queries::get_signing_key();
        claims.sign_with_key(&key).expect("Failed to sign with key!")
    }
}
...
impl<'r> FromRequest<'r> for User {
    ...
    async fn from_request(request: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        if let Some(header) = request.headers().get_one("Authorization") {
            ...
            let key = queries::get_signing_key();
            let claims: RegisteredClaims = string_token.verify_with_key(&key).expect("Failed to verify token!");
            let claims = claims.subject.expect("No claims in token!");
            let user: User = serde_json::from_str(claims.as_str()).expect("Failed to deserialize user!");
